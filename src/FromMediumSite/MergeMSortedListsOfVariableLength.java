...
public class MergeMSortedListsOfVariableLength  implements  Comparable{
    ...
    // A class for heap node
        ...
        // Constructor
        public MergeMSortedListsOfVariableLength(int value, int i, int index) {
            this.value = value;
            this.i = i;
            this.index = index;
        }
        ...
        public int compareTo(Object o) {
            MergeMSortedListsOfVariableLength node = (MergeMSortedListsOfVariableLength)o;
    ...
    class Mergee
    {
        ...
        public static void printSorted(List<List<Integer>> list)
        {
            ...
            PriorityQueue<MergeMSortedListsOfVariableLength> pq = new PriorityQueue();

            // push first element of each list into the min-heap
            // along with list number and their index in the list
            for (int i = 0; i < list.size(); i++) {
                if (list.get(i).size() >= 1) {
                    pq.add(new MergeMSortedListsOfVariableLength(list.get(i).get(0), i, 0));
                }
            }
            ...
            while (!pq.isEmpty())
            {
                ...
                MergeMSortedListsOfVariableLength min = pq.poll();
